...
export default function ResponsiveDashboard({setIsOpen}) {
  ...
  return (
	...
	<div className="flex items-center p-2 space-x-4">
		...
		<div>
			...
			<span className="flex items-center space-x-1">
				<Link to='/dashboard' onClick={()=> setIsOpen(false)}   className="text-xs hover:underline dark:text-gray-600">View profile</Link>
			</span>
		</div>
	</div>
	<div className="divide-y dark:divide-gray-300">
		{/* admin route start ============================================= */}
			{
				...
				<li>
					<Link to='/dashboard/manageUsers' onClick={()=> setIsOpen(false)}  className="flex items-center p-2 space-x-3 rounded-md">
				...
				<li>
					<Link to='/dashboard/addMeal' onClick={()=> setIsOpen(false)}    className="flex items-center p-2 space-x-3 rounded-md">
				...
				<li>
					<Link to='/dashboard/allMeals' onClick={()=> setIsOpen(false)}   className="flex items-center p-2 space-x-3 rounded-md">
						<BiFoodMenu className="w-5 h-5 fill-current dark:text-gray-600" />
						<span>All Meals</span>
					</Link>
				</li>
				<li>
					<Link to='/dashboard/allReviews' onClick={()=> setIsOpen(false)}   className="flex items-center p-2 space-x-3 rounded-md">
						<MdOutlineReviews className="w-5 h-5 fill-current dark:text-gray-600" />
						<span>All Reviews</span>
					</Link>
				</li>
				<li>
					<Link to='/dashboard/serveMeals' onClick={()=> setIsOpen(false)}    className="flex items-center p-2 space-x-3 rounded-md">
						<LuAlignVerticalDistributeEnd className="w-5 h-5 fill-current dark:text-gray-600"  />
						<span>Serve Meals</span>
					</Link>
				</li>
				<li>
					<Link to='/dashboard/upcomingMeals'  onClick={()=> setIsOpen(false)}   className="flex items-center p-2 space-x-3 rounded-md">
        ...
        {/* user route start ==================================================== */} 
        {
			role === 'User' && <ul className="pt-2 pb-4 space-y-1 text-sm">
			<li className="dark:bg-gray-100 dark:text-gray-900">
        ...
        {/* for pages ========================== */}
            ...
            {/* <li>
				<Link to='/upcomingMeals'  className="flex items-center p-2 space-x-3 rounded-md">
