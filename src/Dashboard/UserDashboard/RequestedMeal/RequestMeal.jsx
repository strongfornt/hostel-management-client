...
export default function RequestMeal() {
  ...
  const { data: requestMealsData = [], isLoading } = useQuery({
    queryKey: ["request_meals", currentPage, itemPerPages],
    queryFn: async () => {
      const { data } = await axiosSecure.get(
        `/request_meals/${user?.email}?page=${currentPage}&size=${itemPerPages}`
      );
      return data;
    },
  });
  ...
  useEffect(() => {
    const getCount = async () => {
      const { data } = await axiosPublic.get(
        `/request_meals_count/${user?.email}`
      );
      setCount(data?.requestMealsCount);
    };
    getCount();
