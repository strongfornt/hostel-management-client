...
export default function MyReviews() {
  ...
  const { data: reviewsData = [], isLoading } = useQuery({
    queryKey: ["review", currentPage, itemPerPages],
    queryFn: async () => {
      const { data } = await axiosSecure.get(
        `/reviews/${user?.email}?page=${currentPage}&size=${itemPerPages}`
      );
      return data;
    },
  });
      ...
      <section className="my-7 md:my-10  md:ml-[15rem] lg:ml-1  xl:ml-44">
        <div>
          ...
          <p className="text-sm text-center mb-8 max-w-screen-sm mx-auto text-[#4b5664] ">
            View and manage your reviews in one place. Easily edit, delete, or
            view meals associated with your feedback, along with tracking likes
            on your reviews.
          </p>
        </div>
          ...
          <div className="container  mx-auto  text-gray-100 dark:text-gray-800">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  {reviewsData?.map((meal, idx) => (
                    <MyReviewsTable key={idx} meal={meal} />
                  ))}
                </tbody>
              </table>
            </div>
